...
Battle::AbilityEffects::OnSwitchIn.add(:ORICHALCUMPULSE,
  proc { |ability, battler, battle, switch_in|
    if [:Sun, :HarshSun].include?(battler.effectiveWeather)
      battle.pbShowAbilitySplash(battler)
      battle.pbDisplay(_INTL("{1} is sending its ancient pulse into a frenzy!", battler.pbThis))
      battle.pbHideAbilitySplash(battler)
    else
...
Battle::AbilityEffects::OnSwitchIn.add(:HADRONENGINE,
  proc { |ability, battler, battle, switch_in|
    ...
    if battle.field.terrain == :Electric
      battle.pbDisplay(_INTL("{1} energize its futuristic engine!", battler.pbThis))
      battle.pbHideAbilitySplash(battler)
    else
      battle.pbStartTerrain(battler, :Electric)
      battle.pbDisplay(_INTL("{1} is energizing its futuristic engine!", battler.pbThis))
    end
  }
)
